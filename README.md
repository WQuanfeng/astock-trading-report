...
| 复盘上一日报告 | 自动读取严格 T-1 的同类报告，验证昨日条件是否兑现；不自动改 Skill。 |
| 低位活跃龙头 | 用户指定板块后，筛选“低位活跃龙头候选池”，最多两只且可返回零只。 |
| 上升初段标的 | 在用户指定板块内筛选“趋势延续观察候选”，不称为高概率或必涨。 |
...
交易环境；分别筛选低位活跃龙头候选和上升初段候选，各最多两只。
...
reports/YYYY-MM-DD/sector-<板块>-<模式>-YYYYMMDD.html
```
...
只有用户明确给出板块才运行。低位活跃龙头使用历史活跃交易、历史板块
强势代理信号、当前低位/筑底、流动性与风险过滤；上升初段使用均线趋势、
适度涨幅、未涨停、量能、板块确认与风险过滤。
