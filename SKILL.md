...
description: >
  ...
  报告。用于大盘复盘、个股分析、指定板块低位活跃龙头筛选和可验证股票池中的
  上升初段候选发现。先使用已安装的 a-stock-data 获取数据，再由 Agent 完成分析与报告；
...
| T-1 context JSON 无效 | 忽略旧报告正文，将结构化复盘标为 `not_evaluable`，并继续使用当日数据。 |
...
v2 schema 和结果标签；结构化复盘只用于解释今日变化，绝不自动改动本 Skill、阈值
或 references。
...
3. 不得用单次资金流作为长期投资逻辑。
4. 不得把候选称为“买入”。HTML 使用 `observe`、`eligible if triggered`、
   `not suitable to initiate` 或 `insufficient data`；context JSON 使用
   `references/report-contract.md` 定义的对应枚举值。
...
9. T+1 仅复盘已记录的结构化原子条件；自然语言理由可以解释结论，但不得作为
   可验证触发条件。
