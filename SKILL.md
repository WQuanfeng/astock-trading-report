...
description: >
  ...
  可验证股票池中的上升初段候选发现。先使用已安装的 a-stock-data 获取数据，再由
  Agent 完成分析与报告；
...
| `stock` | `references/market-workflow.md`，再读 `references/stock-workflow.md` | 大盘环境下的单只股票证据与条件化计划 |
| `low-active-leader` | `references/market-workflow.md`，再读 `references/screening-rules.md` | 大盘环境下指定板块内至多两只候选 |
| `early-trend` | `references/market-workflow.md`，再读 `references/screening-rules.md` | 大盘环境下可验证股票池内至多两只候选 |
