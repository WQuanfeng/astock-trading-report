# 大盘每日工作流

## 使用 a-stock-data 收集

对选定的已完成交易日，收集：

- 上证综指、深证成指、创业板指、沪深 300，以及与用户指定板块显著相关的
  指数。
- 指数涨跌、可获得时的日内振幅、成交额及其 T-1 变化。
- 上涨/下跌家数、涨停数、跌停数、炸板数量/比例、最高连板高度、昨日涨停
  表现；对每项可获得指标收集 T-1 变化，并在可用时收集首板/连板结构。
- 领涨和领跌行业/概念，不只看最高涨幅，也看广度；比较其与 T-1 的排名和
  广度，以区分延续与一日轮动。
- 重要市场新闻、政策发布和宏观事件，并保留来源和发布时间。
- 可获得的资金数据；仅作辅助，不得单独作为结论依据。
- `behavioral-workflow.md` 所需的可得市场生态、事件时点和分钟级时序；数据源
  不支持时记录缺口，不得用日线补写盘中路径。

## 按以下顺序解读

1. **趋势与流动性**：解释指数方向、成交额和市场广度相对 T-1 的变化，
   不得只依赖绝对值。
2. **广度与情绪**：上涨/下跌家数、涨停生态、炸板率和上涨集中度。
3. **群体交易行为**：按 `behavioral-workflow.md` 区分追涨、风险规避、分歧
   和事件接受度，只保留可验证结论。
4. **主线**：最强主题是在延续、扩散、轮动还是走弱。
5. **风险事件**：可能影响隔夜的政策、全球市场或披露风险。
6. **T-1 结构化复盘**：仅复盘昨日记录的原子情景和行为观察条件。

## 必须输出

先给出市场状态，再恰好给出三种次日条件化情景：

| 情景 | 需观察的条件 | 解读 | 新开仓环境 |
| --- | --- | --- | --- |
| 积极情景（风险偏好回升） | 具体的开盘、量能、广度和主线条件 | 为什么支持该判断 | 选择性观察 / 条件满足后可介入 |
| 中性情景（震荡或轮动） | 具体的混合条件 | 为什么可能震荡或轮动 | 仅关注高质量形态 |
| 防守情景（风险偏好下降） | 具体的恶化条件 | 为什么应降低风险 | 不适合新开仓 |

报告必须给出区分情景的开盘后 30–60 分钟观察项。每个情景至少包含一个可衡量
触发条件及明确比较基准，例如前一交易日同时间段成交额或近期同时间段中位数；
用于次日结论的条件必须同时写入 context 的原子条件对象。
按以下优先级判断：先判 `risk-off`；仅当所有 `risk-on` 触发条件满足且没有
`risk-off` 条件时判为 `risk-on`；其他情况均为 `neutral`。不得把情景转化为
下单指令。

上述英文值只写入 context JSON。HTML 正文和表格必须使用“积极情景（风险偏好
回升）”“中性情景（震荡或轮动）”和“防守情景（风险偏好下降）”，不得显示
`risk-on`、`neutral` 或 `risk-off`。
