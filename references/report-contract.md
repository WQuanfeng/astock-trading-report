...
| `PROVENANCE_TABLE` | 来源/数据日期/发布时间或可用时间/抓取时间/时点安全性/单位/复权口径表；HTML 片段。 |
...
个股和候选另行显示可交易性，不得把研究状态当作可成交保证：

| HTML 显示 | JSON `tradability.status` |
| --- | --- |
| 当前可正常观察 | `tradable` |
| 当前交易受限，次日需复核 | `restricted` |
| 可交易性未知 | `unknown` |
...
T+1 相对预设基准结果使用下列中文显示：

| HTML 显示 | JSON `relative_outcome` |
| --- | --- |
| 跑赢基准 | `outperformed` |
| 持平基准 | `matched` |
| 跑输基准 | `underperformed` |
...
`behavior-{candidate-id}-{behavior_type}`。筛选审计使用
`screening-audit-early-trend` 或
`screening-audit-low-active-leader-{normalized-sector}`。`report_id` 使用
`{report_type}-{YYYYMMDD}`；
...
```json
{
  ...
  "as_of": "2026-07-15T17:00:00+08:00",
  "report_type": "daily",
  "scope": {
    "symbols": ["600000"],
    "sector": "储能",
...
`["sh_main", "sz_main", "chinext"]`。单一筛选报告继续在 `scope` 填写来源、
标签、总数和已评价数，并与唯一的 `screening_audits[]` 项一致。`daily` 同时运行
两种模式时，报告级 `scope` 的筛选字段使用 `null`，分别以 audit 项为准。

新版 v3 报告必须写入 `screening_audits` 和 `behavioral_analysis`，无内容时使用
空数组。读取缺少这些字段或新增决策完整性字段的早期 v3 报告时，其他结构仍可
复盘，但覆盖、时点完整性、可交易性或相对结果显示为“无法评价”。

### `screening_audits[]`

每种实际执行的候选筛选恰好写入一项；`daily` 可同时包含两项，未执行筛选时使用
空数组。规则排除和数据失败按 `decision-integrity-rules.md` 分开计数。

```json
{
  "id": "screening-audit-early-trend",
  "mode": "early-trend",
  "sector": null,
  "market_scope": ["sh_main", "sz_main", "chinext"],
  "universe_source": "a-stock-data 当次可枚举股票池",
  "universe_label": "完成必要数据检查后的已评价子集",
  "universe_total": 3200,
  "excluded_by_reason": {
    "st_or_delisting_risk": 120,
    "suspended": 20,
    "insufficient_history": 80,
    "low_liquidity": 150,
    "major_event_risk": 10
  },
  "universe_eligible": 2820,
  "data_failures_by_reason": {
    "missing_kline": 10,
    "missing_ranking_metric": 5
  },
  "universe_evaluated": 2805,
  "coverage_rate": 0.9947
}
```

`mode` 只使用 `early-trend` 或 `low-active-leader`。前者的 `sector` 为 `null`；
后者使用规范板块名，且 ID 为 `screening-audit-low-active-leader-{normalized-sector}`。
每只股票只计入一个首要原因；数量无法满足两条对账等式时，不得输出最终候选。
...
```json
{
  ...
  "tradability": {
    ...
    "known_next_session_restriction": false,
    "corporate_action_pending": false,
    "warnings": [],
    "evidence_ids": ["prov-status-1"]
  },
...
`stock_plans[]` 和 `candidates[]` 都必须包含 `tradability`；其状态只使用
`tradable`、`restricted` 或 `unknown`。`suspended`、`special_treatment`、
`known_next_session_restriction` 和 `corporate_action_pending` 使用布尔值，无法
核验时使用 `null`；`price_limit_state` 使用 `none`、`up_locked`、
`down_locked` 或 `unknown`；`liquidity_status` 使用 `normal`、`weak`、
`insufficient` 或 `unknown`。`warnings` 和 `evidence_ids` 不得省略。

`tradability.status` 为 `restricted` 或 `unknown` 时，计划状态不得为
`eligible_if_triggered`。当前价格锁定但没有已知次日限制时，可以保留候选，
但必须设置次日复核原子条件。

两类对象还必须包含 T-1 预先确定的 `evaluation_benchmark`，其 `basis` 只使用
`user_specified`、`primary_industry`、`specified_sector` 或 `broad_market`。
无法取得稳定基准代码或同周期行情时使用 `null`，不得在 T+1 事后补选基准。
...
`avg_amount_20d_cny`，并为产生最大超额收益的窗口记录
`sector_index_symbol`、`leadership_window_start`、`leadership_window_end`、
`stock_return_20d_pct` 和 `sector_return_20d_pct`。最终两只的所有对应值均为必填，
不得用自然语言替代。
...
```json
{
  ...
  "benchmark_reference_level": 4000.0,
  "benchmark_t1_close": 4020.0,
  "benchmark_return_pct": 0.5,
  "excess_return_pct": 1.3868,
  "relative_outcome": "outperformed|matched|underperformed|not_evaluable",
  "condition_results": [
    {
  ...
  "evidence_ids": ["prov-quote-2", "prov-index-2"],
  "note": "一句事实说明"
}
```
...
基准字段仅用于 T-1 已记录 `evaluation_benchmark` 的个股和候选。基准参考值必须
与 `triggered_at` 使用同一最小可靠周期；按 `decision-integrity-rules.md` 计算
`benchmark_return_pct`、`excess_return_pct` 和 `relative_outcome`。缺少预设基准、
同周期数据或参考值时，相关数值使用 `null`，相对结果使用 `not_evaluable`；不得
影响绝对 `selection_outcome`。
...
`source`、`data_date`、`published_at`、`available_at`、`fetched_at`、
`point_in_time_safe`、`unit` 和 `adjustment`。`published_at` 不适用时使用
`null`；`available_at` 无法核验时使用 `null`，同时将 `point_in_time_safe` 设为
`false`。只有时点安全的证据 ID 可以被筛选、排名、计划或行为结论引用。无内容时
使用空数组；不得省略必填数组。
